...
const BottomSheet = ({
  ...
  function getSheetContent(type) {
    switch (type) {
      ...
      case 'tag':
        return <HashTagContent onComplete={handleCompleteTags} initialTags={tempTags} />;
      case 'addSchedule':
        return (
          <AddSchedule
      ...
      case 'scheduleDetail':
        return (
          <ScheduleDetail onDotsClick={onDotsClick} selectedSchedule={selectedSchedule} />
        );
      case 'editDelete':
        return (
          <EditDeleteBottomSheet show={true} onClose={handleCloseBottomSheet} onEditClick={onEditClick} />
        );
      case 'editSchedule':
        return (
          <ScheduleEdit schedule={selectedSchedule} onClose={handleCloseBottomSheet} />
        );
      case 'map':
        return <h1>지도리스트</h1>;
      case '강아지':
      ...
      case '특수동물':
        return (
          <div className="bottom-sheet-buttons">
            <button className="bottom-sheet-button" onClick={() => handleSelectPet('강아지')}>
              강아지
            </button>
            <button className="bottom-sheet-button" onClick={() => handleSelectPet('고양이')}>
              고양이
            </button>
            <button className="bottom-sheet-button" onClick={() => handleSelectPet('특수동물')}>
              특수동물
            </button>
          </div>
        );
