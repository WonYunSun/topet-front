...
const AnimalBirth = ({ year, month, day, setYear, setMonth, setDay, selectedBirth, setSelectedBirth, setNextPossible, birthDontKnow, setBirthDontKnow }) => {
    ...
    const renderYearOptions = () => {
        return years
            ...
            .sort((a, b) => b - a)
            .map((year, index) => (
                <div key={index} className={styles.option} onClick={() => handleYearChange(year)}>
                    {year}년
                </div>
            ));
    };
    ...
    const renderMonthOptions = () => {
        ...
        return months.map((month) => (
            <div key={month} className={styles.option} onClick={() => handleMonthChange(month)}>
                {month}월
            </div>
        ));
    };
    ...
    const renderDayOptions = () => {
        ...
        return days.map((day) => (
            <div key={day} className={styles.option} onClick={() => handleDayChange(day)}>
                {day}일
            </div>
        ));
    };
    ...
    return (
        <div className={styles.wrapper}>
            ...
            <div className={styles.birth_wrapper}>
                <div className={`${styles.custom_select} ${styles.year_select} ${birthDontKnow ? styles.disabled : ''}`}>
                    <div className={styles.selected_option} onClick={() => toggleDropdown('year')}>
                        {year ? `${year}년` : '년'}
                        <TbTriangleInvertedFilled className={styles.dropdown_icon} />
                    </div>
                    <div className={`${styles.options} ${dropdown.year ? styles.show : ''}`}>
                        {renderYearOptions()}
                    </div>
                </div>
                <div className={`${styles.custom_select} ${styles.month_day_select} ${birthDontKnow || !year ? styles.disabled : ''}`}>
                    <div className={styles.selected_option} onClick={() => toggleDropdown('month')}>
                        {month ? `${month}월` : '월'}
                        <TbTriangleInvertedFilled className={styles.dropdown_icon} />
                    </div>
                    <div className={`${styles.options} ${dropdown.month ? styles.show : ''}`}>
                        {renderMonthOptions()}
                    </div>
                </div>
                <div className={`${styles.custom_select} ${styles.month_day_select} ${birthDontKnow || !month ? styles.disabled : ''}`}>
                    <div className={styles.selected_option} onClick={() => toggleDropdown('day')}>
                        {day ? `${day}일` : '일'}
                        <TbTriangleInvertedFilled className={styles.dropdown_icon} />
                    </div>
                    <div className={`${styles.options} ${dropdown.day ? styles.show : ''}`}>
                        {renderDayOptions()}
                    </div>
                </div>
            </div>
